...
class Steerable:
    ...
    def buildSFpyr(self, im):
        ...
        coeff = self.buildSFpyrlevs(lo0dft, log_rad, angle, Xrcos, Yrcos,
                                    self.height - 1)
    ...
    def buildSFpyrlevs(self, lodft, log_rad, angle, Xrcos, Yrcos, ht):
        if (ht <= 1):
            ...
            Xcosn = np.pi * np.array(range(-(2 * lutsize + 1),
                                           (lutsize + 2))) / lutsize
            order = self.nbands - 1
            const = (2**(2 * order) * sc.factorial(order)**2) / (
            ...
            for b in range(int(self.nbands)):
                anglemask = self.pointOp(angle, Ycosn, Xcosn +
                                         (np.pi * b) / self.nbands).astype(
                                             np.complex)
                banddft = (np.complex(0, -1)**order) * lodft
                banddft *= anglemask
                banddft *= himask
            ...
            coeff = self.buildSFpyrlevs(lodft, log_rad, angle, Xrcos, Yrcos,
                                        ht - 1)
    ...
    def reconSFPyrLevs(self, coeff, log_rad, Xrcos, Yrcos, angle):
        if (len(coeff) == 1):
            ...
            for b in range(int(self.nbands)):
                anglemask = self.pointOp(angle, Ycosn,
                                         Xcosn + (np.pi * b) / self.nbands)
                banddft = np.fft.fftshift(np.fft.fft2(coeff[0][b]))
                orientdft += ((np.complex(0, 1)**
                               (order)) * banddft * anglemask * himask)
            ...
            nresdft = self.reconSFPyrLevs(coeff[1:], nlog_rad, Xrcos, Yrcos,
                                          nangle)
    ...
    def normalize(self, coef, height, order):
        ...
        for pyr_h in xrange(height - 2):
            ...
            for cband in xrange(order):
                ...
                if pyr_h > 0:
                    child = skimage.transform.rescale(
                        coef[pyr_h][cband], scale=0.5, order=1)
                if pyr_h + 3 < height:
                    #parent = scipy.misc.imresize(coef[pyr_h+2][cband], 2.0, interp='bilinear', mode='F')
                    parent = skimage.transform.rescale(
                        coef[pyr_h + 2][cband],
                        scale=2.0,
                        order=1)
                ...
                idx = np.hstack((np.arange(0, cband),
                                 np.arange(cband + 1, order)))
                ...
                if parent == []:
                    cov = np.array(
                        np.hstack((
                            ...
                            rolling_window(sublevel[cband], filtsize).reshape(
                                ((w - 2) * (h - 2), 9)),
                            #grab coefficients from neighboring orientations
                            sublevel.transpose(1, 2, 0)[1:-1, 1:-1, idx]
                            .reshape((w - 2) * (h - 2), order - 1),
                ...
                z = np.sqrt(
                    np.einsum('ij,ij->i', np.dot(cov, cov_inv), cov) / N)
...
def load_sp5filters():
    ...
    mtx = np.array([[0.3333, 0.2887, 0.1667, 0.0000, -0.1667, -0.2887], [
    ...
    hi0filt = np.array([[
        -0.00033429, -0.00113093, -0.00171484, -0.00133542, -0.00080639,
        -0.00133542, -0.00171484, -0.00113093, -0.00033429
    ], [
        -0.00113093, -0.00350017, -0.00243812, 0.00631653, 0.01261227,
        0.00631653, -0.00243812, -0.00350017, -0.00113093
    ], [
        -0.00171484, -0.00243812, -0.00290081, -0.00673482, -0.00981051,
        -0.00673482, -0.00290081, -0.00243812, -0.00171484
    ], [
        -0.00133542, 0.00631653, -0.00673482, -0.07027679, -0.11435863,
        -0.07027679, -0.00673482, 0.00631653, -0.00133542
    ], [
        -0.00080639, 0.01261227, -0.00981051, -0.11435863, 0.81380200,
        -0.11435863, -0.00981051, 0.01261227, -0.00080639
    ], [
        -0.00133542, 0.00631653, -0.00673482, -0.07027679, -0.11435863,
        -0.07027679, -0.00673482, 0.00631653, -0.00133542
    ], [
    ...
    lofilt = 2 * np.array([[
        0.00085404, -0.00244917, -0.00387812, -0.00944432, -0.00962054,
        -0.00944432, -0.00387812, -0.00244917, 0.00085404
    ], [
    ...
    bfilts = np.array([[[
        0.00277643, 0.00496194, 0.01026699, 0.01455399, 0.01026699, 0.00496194,
        0.00277643
    ], [
        -0.00986904, -0.00893064, 0.01189859, 0.02755155, 0.01189859,
        -0.00893064, -0.00986904
    ], [
        -0.01021852, -0.03075356, -0.08226445, -0.11732297, -0.08226445,
        -0.03075356, -0.01021852
    ], [
        0.00000000, 0.00000000, 0.00000000, 0.00000000, 0.00000000, 0.00000000,
        0.00000000
    ], [
        0.01021852, 0.03075356, 0.08226445, 0.11732297, 0.08226445, 0.03075356,
        0.01021852
    ], [
        0.00986904, 0.00893064, -0.01189859, -0.02755155, -0.01189859,
        0.00893064, 0.00986904
    ], [
        -0.00277643, -0.00496194, -0.01026699, -0.01455399, -0.01026699,
        -0.00496194, -0.00277643
    ]], [[
        -0.00343249, -0.00640815, -0.00073141, 0.01124321, 0.00182078,
        0.00285723, 0.01166982
    ], [
        -0.00358461, -0.01977507, -0.04084211, -0.00228219, 0.03930573,
        0.01161195, 0.00128000
    ], [
        0.01047717, 0.01486305, -0.04819057, -0.12227230, -0.05394139,
        0.00853965, -0.00459034
    ], [
        0.00790407, 0.04435647, 0.09454202, -0.00000000, -0.09454202,
        -0.04435647, -0.00790407
    ], [
        0.00459034, -0.00853965, 0.05394139, 0.12227230, 0.04819057,
        -0.01486305, -0.01047717
    ], [
        -0.00128000, -0.01161195, -0.03930573, 0.00228219, 0.04084211,
        0.01977507, 0.00358461
    ], [
        -0.01166982, -0.00285723, -0.00182078, -0.01124321, 0.00073141,
        0.00640815, 0.00343249
    ]], [[
        0.00343249, 0.00358461, -0.01047717, -0.00790407, -0.00459034,
        0.00128000, 0.01166982
    ], [
        0.00640815, 0.01977507, -0.01486305, -0.04435647, 0.00853965,
        0.01161195, 0.00285723
    ], [
        0.00073141, 0.04084211, 0.04819057, -0.09454202, -0.05394139,
        0.03930573, 0.00182078
    ], [
        -0.01124321, 0.00228219, 0.12227230, -0.00000000, -0.12227230,
        -0.00228219, 0.01124321
    ], [
        -0.00182078, -0.03930573, 0.05394139, 0.09454202, -0.04819057,
        -0.04084211, -0.00073141
    ], [
        -0.00285723, -0.01161195, -0.00853965, 0.04435647, 0.01486305,
        -0.01977507, -0.00640815
    ], [
        -0.01166982, -0.00128000, 0.00459034, 0.00790407, 0.01047717,
        -0.00358461, -0.00343249
    ]], [[
        -0.00277643, 0.00986904, 0.01021852, -0.00000000, -0.01021852,
        -0.00986904, 0.00277643
    ], [
        -0.00496194, 0.00893064, 0.03075356, -0.00000000, -0.03075356,
        -0.00893064, 0.00496194
    ], [
        -0.01026699, -0.01189859, 0.08226445, -0.00000000, -0.08226445,
        0.01189859, 0.01026699
    ], [
        -0.01455399, -0.02755155, 0.11732297, -0.00000000, -0.11732297,
        0.02755155, 0.01455399
    ], [
        -0.01026699, -0.01189859, 0.08226445, -0.00000000, -0.08226445,
        0.01189859, 0.01026699
    ], [
    ...
    return lo0filt.astype(np.float32), hi0filt.astype(
        np.float32), lofilt.astype(np.float32), bfilts.astype(
            np.float32), mtx.astype(np.float32), harmonics.astype(np.float32)
...
class SpatialSteerablePyramid():
    ...
    def corr(self, A, fw):
        ...
        newpad = np.vstack((A[1:fw.shape[0] - sy2, :][::-1], A,
                            A[h - (fw.shape[0] - sy2):h - 1, :][::-1]))  #,
        newpad = np.hstack((newpad[:, 1:fw.shape[1] - sx2][:, ::-1], newpad,
                            newpad[:, w - (fw.shape[1] - sx2):w - 1][:, ::-1]))
        newpad = newpad.astype(np.float32)

        return scipy.signal.correlate2d(
            newpad, fw, mode='valid').astype(np.float32)
    ...
    def buildLevs(self, lo0, lofilt, bfilts, edges, mHeight):
        ...
        bands = [bands] + self.buildLevs(lo, lofilt, bfilts, edges,
                                         mHeight - 1)
    ...
    def decompose(self, inputimage, filtfile='sp1Filters', edges='symm'):
        ...
        if filtfile == 'sp5Filters':
            ...
            raise (NotImplementedError,
                   "That filter configuration is not implemnted")
